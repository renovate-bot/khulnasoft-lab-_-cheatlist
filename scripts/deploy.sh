...
function initialize {
  ...
  export SITE_REPO_SLUG="khulnasoft-lab/cheatlist-pages.github.io"
...
function upload_assets {
  ...
  sha256sum \
    ...
    > "${SITE_HOME}/assets/khulnasoft-lab.github.io/cheatlista256sums"

  cd "$SITE_HOME"
  git add -A
  git commit -m "[GitHub Actions] uploaded assets after commit khulnasoft-lab/cheatlist@${GITHUB_SHA}"
  git push -q
